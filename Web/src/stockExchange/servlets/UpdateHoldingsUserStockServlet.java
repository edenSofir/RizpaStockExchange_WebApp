...
public class UpdateHoldingsUserStockServlet extends HttpServlet {
    protected void processRequest(HttpServletRequest request , HttpServletResponse response) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            User thisUser = users.findUserByUserName(usernameFromSession);
            Stock stock =(Stock) request.getSession().getAttribute("stock");
            Item userHoldingsInStock = thisUser.getHoldings().findItemByStockSymbol(stock.getSymbol());
            String json = gson.toJson(userHoldingsInStock);
            out.println(json);
            out.flush();
        }
    }
