...
public class UserMessageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request , HttpServletResponse response) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            Gson gson = new Gson();
            User user = ServletUtils.getEngine(request.getServletContext()).getUsers().getUsersList().get(SessionUtils.getUsername(request));
            Queue<String> messageInfo = user.getMessageInfo();
            Queue<String> tmp = new PriorityQueue<>();
           while(!messageInfo.isEmpty())
           {
               tmp.add(messageInfo.poll());
           }
